...
const createBlog = async function (req, res) {
  try {
    ...
    let { title, body, authorId, tags, subcategory, category, isPublished, deletedAt } = data
    ...
    data.publishedAt = data.isPublished ? new Date() : " ";
    if (data.isDeleted) {
      deletedAt = " ";
    }
...
const getBlogs = async function (req, res) {
  try {
    ...
    if (blogs.authorId != req.decodedToken.authorId) {
      ...
      if (!check(blogs)) {
        return res.status(400).send({ status: true, msg: "blogs not found" });
      } else if (check(blogs)) {
        ...
        if (searchBlogs.length > 0) {
          for (let element of searchBlogs) {
            if (element.isDeleted == false && element.isPublished == true) {
              //if(element.isDeleted==false?element.deleteAt==" ":element.deleteAt== new Date())
              result.push(element);
            }
          }
...
const deleteBlogById = async function (req, res) {
  try {
    ...
    if (!isValidObjectId(blogId)) {
      return res.status(400).send({status:false, msg:"blog id not entered"})
...
const deleteBlogByParams = async function(req, res) {
  ...
  try {
    ...
    if (!body) {
      ...
      return res.status(404).send({ status: false, msg: "No such blog exists or the blog is deleted" })
    }

    let{title, category, authorId, tags, subcategory, isPublished} = body; // take all the element in the body container
        if (title) {
          if (!isValid(title)) {
            return res.status(400).send({ status: false, msg: "No such title found" })
      ...
      if (authorId) { 
          if(!isValidObjectId(authorId))
            return res.status(400).send({ status: false, msg: "No such author found" });
        }

      if (tags) {
          if(!check(tags))
            return res.status(400).send({ status: false, msg: "No such" })
        }

      if (subcategory) {
          if(!check(subcategory))
            return res.status(400).send({ status: false, msg: "no such subcategory" })
        }
        if (isPublished==true) {
        ...
        let deleteBlog = await blogModel.findOneAndUpdate({ _id: { $in: blog } }, { $set: { isDeleted: true },deletedAt: new Date() }, { new: true })
